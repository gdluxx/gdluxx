...
function createApiKeyStore() {
	...
	const handleApiError = async (
		...
		const errorMsg = errorData.error || errorData.message || `${fallbackErrorPrefix}: ${response.statusText}`;
		setError(errorMsg);
	...
	return {
		...
		load: async (): Promise<ApiKeyActionResult> => {
			...
			try {
				...
				setApiKeys(apiKeys);
				return { success: true, message: 'API keys loaded successfully', data: apiKeys, type: 'info' };
			} catch (e) {
