...
<script lang="ts">
  ...
  const supportedSites = $state<SupportedSite[]>(data.supportedSites || []);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (editingConfig) {
          ...
          if (index >= 0) {configs[index] = result.data.config;}
        } else {
  ...
  function getPriorityLabel(priority: number): string {
    if (priority >= 1000) {return 'Global';}
    if (priority >= 500) {return 'Domain';}
    return 'Specific';
  }

  function getPriorityColor(priority: number): string {
    if (priority >= 1000) {return 'text-blue-600 dark:text-blue-400';}
    if (priority >= 500) {return 'text-green-600 dark:text-green-400';}
...
<PageLayout
  ...
  description="Configure CLI options for specific sites when using the browser extension"
>
  {#snippet icon()}
  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-secondary-500 dark:text-secondary-500 mt-1">
          Click "Refresh Sites" to fetch the latest supported sites from gallery-dl.
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      >
        ...
        <p class="text-3xl font-bold text-primary-600 dark:text-primary-400">{configs.length}</p>
      </div>
      <div
        class="bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-lg p-4"
      >
        ...
        <p class="text-3xl font-bold text-green-600 dark:text-green-400">{supportedSites.length}</p>
      </div>
      <div
        class="bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-lg p-4"
      >
        <h3 class="text-lg font-semibold text-secondary-900 dark:text-secondary-100">Categories</h3>
    ...
    <div class="space-y-4">
      {#each configs as config}
        <div
          class="bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-lg p-4 flex justify-between items-center"
        >
          <div class="flex-1">
          ...
          <div class="flex gap-2">
            ...
            <Button onclick={() => handleDeleteConfig(config.id!)} variant="outline-danger">
